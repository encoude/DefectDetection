...
# 将模型转移到 config.py 中指定的设备
device = torch.device(config.device)  # 使用config中的device
...
# 假设 train_losses 和 valid_losses 已记录损失值
# 使用模型训练期间保存的损失和准确率进行绘图

train_losses = [0.5] * 100  # 示例损失值
valid_losses = [0.4] * 100  # 示例损失值
train_accuracies = [0.75] * 100  # 示例准确率
valid_accuracies = [0.78] * 100  # 示例准确率
...
plt.plot(range(100), valid_losses, label='Valid Loss')
...
plt.plot(range(100), valid_accuracies, label='Valid Accuracy')
...
# 保存图表到文件夹中
plt.savefig(os.path.join(folder_name, 'training_results.png'))  # 图表保存为 'training_results.png' 在指定文件夹中
print(f"训练过程的图表已保存至 {folder_name}/training_results.png")

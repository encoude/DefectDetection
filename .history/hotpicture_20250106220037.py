...
def grad_cam(model, img_tensor):
    ...
    def save_gradient(grad):
        nonlocal gradient
        gradient = grad
        print(f"Gradient shape: {gradient.shape if gradient is not None else 'None'}")  # 打印梯度形状
    
    # 保存激活图的 hook
    def save_activation_map(module, input, output):
        nonlocal activation_map
        activation_map = output
        print(f"Activation map shape: {activation_map.shape if activation_map is not None else 'None'}")  # 打印激活图形状

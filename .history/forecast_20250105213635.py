...
def overlay_heatmap(img, heatmap):
    # 将热力图转换为灰度图
    heatmap = cv2.normalize(heatmap, None, 0, 255, cv2.NORM_MINMAX)  # 归一化到 0-255 范围
    heatmap = np.uint8(heatmap)  # 转换为整型

    # 将图像转换为灰度图
    img_gray = np.array(img.convert('L'))  # 转换为灰度图（'L'模式）

    # 创建一个伪彩色化的灰度图，直接叠加
    overlay = cv2.addWeighted(img_gray, 0.7, heatmap, 0.3, 0)

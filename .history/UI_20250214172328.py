...
class TrainingUI(QWidget):
    ...
    def append_log(self, message):
        # 将新的日志内容插入到现有日志的顶部
        self.current_log = message + "\n" + self.current_log  # 在现有日志的顶部插入新消息
        self.log_text.setPlainText(self.current_log)  # 更新日志框内容
        self.log_text.moveCursor(QTextCursor.End)  # 始终确保光标停留在文本框的底部

...
# 创建数据集和数据加载器
train_dataset = DefectDataset(data_dir=config.train_datasetPatn, transform=transform)
...
def evaluate_model(model, dataloader):
    model.eval()
    running_corrects = 0
    total_samples = len(dataloader.dataset)  # 获取数据集的总样本数
    if total_samples == 0:
    ...
    accuracy = float(running_corrects) / total_samples
    print(f'Evaluation Accuracy: {accuracy:.4f}')



# 模型路径
model_path = r'F:\python\AI\DefectDetection\2025.1.5\Defect\trained_model.pth'

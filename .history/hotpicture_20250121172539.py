...
def apply_jet_colormap(heatmap, config.alpha):
    """
...
# 将热力图与原始图像叠加
def overlay_heatmap_with_alpha(img, heatmap, config.alpha):
    """
...
def show_and_save_heatmap(img_path, heatmap, output_dir, alpha):
    """
    显示热力图和原始图像，并保存图像。
    """
    # 确保输出目录存在
    ...
    # 将热力图叠加到原始图像
    overlay = overlay_heatmap_with_alpha(img, heatmap, config.alpha)

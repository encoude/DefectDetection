        ...
        for inputs, labels in dataloader:
            ...
            for idx, pred in enumerate(preds):
                if pred == 1:  # 1 代表 defect
                    ...
                    # 保存原始图像
                    img_name = os.path.basename(img_path)
                    img.save(os.path.join(output_dir, f"defect_{img_name}"))

                    # 保存带缺陷位置的图像
                    defect_image_with_box = img.copy()
                    draw = ImageDraw.Draw(defect_image_with_box)
                    draw.rectangle([left, top, right, bottom], outline="blue", width=3)  # 蓝色框为缺陷位置
                    defect_image_with_box.save(os.path.join(output_dir, f"defect_location_{img_name}"))
    
    print(f"Defect images and their defect location images have been saved to {output_dir}")

# 设置模型路径和输出文件夹
model_path = r'F:\python\AI\DefectDetection\2025.1.5\best_model.pth'
output_dir = r'F:\python\AI\DefectDetection\2025.1.5\defect_images_with_locations'

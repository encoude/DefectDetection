...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    plt.plot(range(num_epochs), valid_accs, label='Validation Accuracy')
    ...
    # 保存图像
    plt.savefig(os.path.join(config.output_dir, 'training_results.png'))
    plt.close()

    # 保存最佳模型
    torch.save(best_model_wts, os.path.join(config.output_dir, 'best_model.pth'))
    print(f"模型已保存至 {os.path.join(config.output_dir, 'best_model.pth')}")

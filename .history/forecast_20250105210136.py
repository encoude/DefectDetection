...
def grad_cam(model, img_tensor):
    ...
    # 获取最后一个卷积层（请根据你的模型结构修改）
    final_conv_layer = model.layer4[1].conv2  # 对于 ResNet18，这可能需要根据你的模型结构调整
    gradient = None
    activation_map = None
    ...
    # 反向传播，计算梯度
    model.zero_grad()
    target_class.backward(retain_graph=True)  # retain_graph=True 可以保持计算图
    
    # 添加调试输出，确保梯度和激活图不为 None

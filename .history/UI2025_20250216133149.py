...
class InferenceUI(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle("缺陷检测模型推理")
        self.setGeometry(200, 200, 600, 400)

        # 输入图片按钮
        self.input_label = QLabel("选择输入图片：")
    ...
    def select_image(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "选择图片", "", "Image Files (*.bmp *.png *.jpg *.jpeg)")
        if file_path:
            self.result_output.setText(f"已选择图片：{file_path}\n推理结果：待推理...")
...
class TrainingUI(QWidget):
    ...
    def init_ui(self):
        ...
        layout.addWidget(self.log_text_train, stretch=1)
        layout.addWidget(self.log_text_user, stretch=1)
        self.setLayout(layout)
    ...
    def open_inference_window(self):
        self.inference_window = InferenceUI()

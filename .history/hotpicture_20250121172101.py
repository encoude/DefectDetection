...
def apply_jet_colormap(heatmap, alpha=0.6):
    """
    ...
    # 使用 Matplotlib 的 jet 映射
    jet_colormap = colormaps.get_cmap('jet')  # 替代过时的 get_cmap
    colored_heatmap = jet_colormap(normalized_heatmap)  # 返回 RGBA 格式
    
    # 去掉 Alpha 通道，并转换为 8-bit 图像
    colored_heatmap = (colored_heatmap[:, :, :3] * 255).astype(np.uint8)
    
    return colored_heatmap

# 将热力图与原始图像叠加
def overlay_heatmap_with_alpha(img, heatmap, alpha=0.6):
    """
    ...
    # 叠加图像与热力图
    overlay = cv2.addWeighted(img, 1 - alpha, jet_heatmap, alpha, 0)
    
    return overlay

def show_and_save_heatmap(img_path, heatmap, output_dir, alpha=1.5):
    """
    显示热力图和原始图像，并保存图像。
    """
    # 确保输出目录存在
    ...
    overlay = overlay_heatmap_with_alpha(img, heatmap, alpha)

    # 提取图像名称
    img_name = os.path.basename(img_path).split('.')[0]  # 去掉文件后缀
    
    # 保存原始图像和叠加的热力图
    img.save(os.path.join(output_dir, f"{img_name}_original.bmp"))
    overlay_img = Image.fromarray(overlay)
    overlay_img.save(os.path.join(output_dir, f"{img_name}_heatmap.bmp"))
        ...
        for inputs, labels in dataloader:
            ...
            for idx, pred in enumerate(preds):
                if pred == 1:  # 1 代表 defect
                    ...
                    # 将热力图叠加到原始图像
                    overlay_heatmap_with_alpha(img, heatmap)

                    # 显示热力图
                    show_and_save_heatmap(img_path, heatmap, output_dir)

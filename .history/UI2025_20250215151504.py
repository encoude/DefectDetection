...
def train_model(ui, model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ui.log_text_train.append(f'Epoch {epoch}/{num_epochs - 1}')
        for phase in ['train', 'valid']:
            ...
            ui.log_text_train.append(f'{phase} Loss: {epoch_loss:.4f} Acc: {epoch_acc:.4f}')
...
class TrainingUI(QWidget):
    ...
    def select_config_file(self):
        ...
        if file_path:
            self.append_user_log(f"已选择配置文件：{file_path}")
    ...
    def start_training(self):
        ...
        self.append_user_log(f"训练轮次设置为: {num_epochs}轮")
